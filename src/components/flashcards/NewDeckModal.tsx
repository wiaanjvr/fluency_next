...
/** Convert a steps array like [1, 10] → "1 10" for display */
function stepsToString(steps: number[]): string {
  return steps.join(" ");
}

/** Parse a space/comma-separated string like "1 10" → [1, 10], filtering invalid */
...
export function NewDeckModal({ open, onClose, onSubmit }: NewDeckModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewPerDay(20);
      setReviewPerDay(100);
      setLearningStepsRaw("1 10");
      setGraduatingInterval(1);
      setEasyInterval(4);
      setInsertionOrder("random");
      setShowAdvanced(false);
      setMaxInterval(36500);
      setIntervalModifier(1.0);
      setHardIntervalMult(1.2);
      setEasyBonus(1.3);
      setShowReviews(false);
      setRelearningStepsRaw("10");
      setMinIntervalAfterLapse(1);
      setNewIntervalMultiplier(0);
      setLeechThreshold(8);
      setLeechActionValue("tag");
      setShowLapses(false);
      setNewGatherOrder("deck_order");
      setNewSortOrder("order_gathered");
      setReviewSortOrder("due_date");
      setInterleaveMode("mix");
      setShowDisplayOrder(false);
      setBuryNewSiblings(false);
      setBuryReviewSiblings(false);
      setShowBurying(false);
      setShowAnswerTimer(false);
      setAnswerTimerLimit(60);
      setShowTimerSection(false);
      setAutoAdvanceAnswerSeconds(0);
      setAutoAdvanceRateSeconds(0);
        ...
        <div className="rounded-xl border border-white/10 overflow-hidden">
          ...
          {showBurying && (
            <div className="px-4 pb-4 space-y-4 border-t border-white/10 pt-4">
              ...
              <p className="text-xs text-white/30">
                When enabled, sibling cards are postponed until the next day
                after reviewing one of them.
              </p>
            </div>
          )}
        </div>
          ...
          {showAutoAdvance && (
            <div className="px-4 pb-4 space-y-4 border-t border-white/10 pt-4">
              ...
              <div className="space-y-1.5">
                ...
                />
                <p className="text-xs text-white/30">
                  After revealing the answer, auto-rate as &quot;Good&quot;
